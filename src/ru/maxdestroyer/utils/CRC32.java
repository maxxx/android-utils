...
{
	static int[] CRC32Table = { 0x00000000, 0x77073096, 0xee0e612c, 0x990951ba,
	...
	private int crc32val;

	//
	private int UpdateCRC32(byte val, int crcZ)
	{
		return CRC32Table[(short) (crcZ& 0xFF) ^ val] ^ (crcZ >> 8);
	}

	public int strcrc32(byte[] ptr, int size, int crcZ)
	{
		int off = 0;
		while (size-- > 0)
			crcZ = UpdateCRC32(ptr[off++], crcZ);
		crc32val = crcZ;
		return crcZ;
	}
	
	public static int crc32(byte[] s, int crcVal)
	{
		CRC32 crc = new CRC32();
		crc.crc32val = crcVal;
		crc.update(s);
	...
	public static int crc32(byte[] s)
	{
		CRC32 crc = new CRC32();
		crc.update(s);
		return crc.value();
	...
	public void update(byte[] s, int off, int len)
	{
		for (int i = off; i < len; i++)
			crc32val = CRC32Table[(crc32val ^ s[i]) & 0xff] ^ (crc32val >>> 8);
	}
}
